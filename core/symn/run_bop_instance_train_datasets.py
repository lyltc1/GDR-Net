""" usage: python run_train_dataset.py --config-file /home/lyltc/git/GDR-Net/configs/symn/ycbv/symn_net_config.py
    In this script, we set TRAIN datasets to ("real",) , detail in # debug setting.
    If you want to view pbr dataset, set cfg.DATASETS.TRAIN to ("pbr",).
"""
import math
import numpy as np
...
from core.symn.datasets.BOPDataset_utils import build_train_BOP_instance_dataset
import cv2
...
cfg.DATASETS.TRAIN = ("pbr",)  # ("real",)

# ---- main function build datasets, data is test datasets
data = build_train_BOP_instance_dataset(cfg, cfg.DATASETS.TRAIN, debug)

obj_ids = cfg.DATASETS.OBJ_IDS
res_crop = cfg.DATASETS.RES_CROP
window_names = ['rgb_crop', 'mask_crop', 'mask_visib_crop']
