...
class ConvPnPNet(nn.Module):
    def __init__(self, nIn, rot_dim=6, drop_prob=0.0, dropblock_size=5):
        super().__init__()
        assert rot_dim == 6, rot_dim
        self.featdim = 128
    ...
    def forward(self, mask_prob, binary_code_prob):
        """
        output:
            rot_param: allo_rot6d params
            t_param: SITE params
        """
        x = torch.cat([mask_prob, binary_code_prob], dim=1)
