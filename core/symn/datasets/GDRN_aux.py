...
class AugRgbAux(BopInstanceAux):
...
class ReplaceBgAux(BopInstanceAux):
    ...
    def init(self, dataset: BopInstanceDataset):
        ...
        cache_path = osp.join(dataset.data_folder.parents[2], "cache_{}_{}.pkl".format(self.bg_type, hashed_file_name))
        ...
        if osp.exists(cache_path):
            ...
            return
        if self.bg_type == "VOC":
            VOC_root = dataset.data_folder.parents[2] / "VOCdevkit" / "VOC2012"
            JPEGImagesPath = VOC_root / "JPEGImages"
            img_paths = [osp.join(JPEGImagesPath, fn.name)
                         for fn in os.scandir(JPEGImagesPath) if ".jpg" in fn.name]
        elif self.bg_type == "VOC_table":
            VOC_root = dataset.data_folder.parents[2] / "VOCdevkit" / "VOC2012"
            VOC_table_list_path = VOC_root / "ImageSets/Main/diningtable_trainval.txt"
